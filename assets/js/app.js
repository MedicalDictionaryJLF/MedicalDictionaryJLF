...
const ANAMNESIS_DICTIONARY_CANDIDATE_GROUPS = [
  ...
  [
    DATA_BASE + "app_language/anamnesis_pediatrics.csv",
...
let atcCodesByDrugId = new Map();
let atcCodesByEnglishName = new Map();
let atcCodesBySlovakName = new Map();
...
async function loadFirstAvailableCsv(candidatePaths){
  ...
  for(const path of candidatePaths){
    try{
      const txt = await loadBaseFile(path);
      const rows = parseCSVLines(txt || "");
  ...
  throw lastErr || new Error("CSV file could not be loaded");
}

async function loadAtcCodes(){
  return loadFirstAvailableCsv(PHARMACOLOGY_CODES_FILE_CANDIDATES);
}

async function loadAtcReference(){
  return loadFirstAvailableCsv(PHARMACOLOGY_REFERENCE_FILE_CANDIDATES);
}

async function loadAtcDescriptions(){
  return loadFirstAvailableCsv(PHARMACOLOGY_DESCRIPTION_FILE_CANDIDATES);
}
...
function buildPharmacologyIndex(atcCodeRows, atcReferenceRows, atcDescriptionRows){
  ...
  atcCodesByDrugId = new Map();
  atcCodesByEnglishName = new Map();
  atcCodesBySlovakName = new Map();
  ...
  for(const children of atcReferenceChildrenByParent.values()){
    children.sort((a, b)=> String(a.atc_code || "").localeCompare(String(b.atc_code || "")));
  }
  ...
  const upsertSearchNameMap = (map, nameKey, row)=>{
    if(!nameKey) return;
  ...
  const materializeRecord = (codeRow, descRow)=>{
    ...
    const atcAll = [...new Set([
      atcPrimary,
      ...splitAtcCodes(merged.atc_all)
    ].filter(Boolean))];
    ...
    if(codeRow){
      ...
      atcCodesByDrugId.set(drugId, normalizedCodeRow);
      upsertSearchNameMap(atcCodesByEnglishName, normalizeSearchText(englishName), normalizedCodeRow);
      upsertSearchNameMap(atcCodesBySlovakName, normalizeSearchText(slovakName), normalizedCodeRow);
    }
  ...
  for(const rawCode of atcCodeRows || []){
    ...
    const descRow = pharmacologyDescriptionsByDrugId.get(resolvedId)
  ...
  pharmacologyRecords.sort((a, b)=> String(a.english_name || a.drug_id).localeCompare(String(b.english_name || b.drug_id)));
...
async function ensurePharmacologyIndexLoaded(options = {}){
  const opts = options || {};
  if(opts.clearCache){
    pharmacologyState.loaded = false;
    pharmacologyState.failed = false;
    pharmacologyState.loadPromise = null;
    pharmacologyRecords = [];
    pharmacologyByDrugId = new Map();
    atcCodesByDrugId = new Map();
    atcCodesByEnglishName = new Map();
  ...
  if(pharmacologyState.loaded) return pharmacologyRecords;
  if(pharmacologyState.loadPromise) return pharmacologyState.loadPromise;
  pharmacologyState.loadPromise = (async ()=>{
    try{
      ...
      pharmacologyState.loaded = true;
      pharmacologyState.failed = false;
      return pharmacologyRecords;
    }catch(e){
      pharmacologyState.failed = true;
      console.warn("Pharmacology index load failed:", e && e.message ? e.message : e);
      return [];
    }finally{
      pharmacologyState.loadPromise = null;
    }
  })();
  return pharmacologyState.loadPromise;
}
...
function isCompactAnamnesisLayoutMode(){
  const layout = getResolvedAnamnesisLayoutMode();
  return layout === "tablet" || layout === "phone";
}
...
function syncAnamnesisMobileToolbar(opts = {}){
  ...
  if(!screen || !editorCard || editorCard.classList.contains("hidden") || !isCompactAnamnesisLayoutMode()){
    if(screen){
...
function getResolvedAnamnesisLayoutMode(){
  return anamnesisLayoutMode === "auto" ? detectAnamnesisScreenFit() : anamnesisLayoutMode;
}
...
function initAnamnesisPreferences(){
  try{ anamnesisLayoutMode = normalizeAnamnesisLayoutMode(localStorage.getItem(ANAMNESIS_LAYOUT_MODE_KEY)); }catch(e){ anamnesisLayoutMode = "auto"; }
...
function getAnamnesisRecordSummary(record){
  ...
  const name = String(record && (record.name || shared.ident_full_name) ? (record.name || shared.ident_full_name) : tOr("anamnesis_unnamed_patient", "Unnamed patient")).trim();
...
function loadAnamnesisRegistryFromStorage(){
  ...
  anamnesisPatientRecords = records.sort((a, b)=> String(b.updatedAt || "").localeCompare(String(a.updatedAt || "")));
  const storedActiveId = localStorage.getItem(ANAMNESIS_ACTIVE_PATIENT_KEY) || "";
  activeAnamnesisPatientId = getAnamnesisPatientRecordById(storedActiveId) ? storedActiveId : (anamnesisPatientRecords[0] ? anamnesisPatientRecords[0].id : "");
}
...
function updateAnamnesisEditorChrome(record){
  ...
  const editorCard = document.getElementById("anamnesis-editor-card");
  const title = document.getElementById("anamnesis-editor-title");
  const hasRecord = !!record;
  const isDesktop = getResolvedAnamnesisLayoutMode() === "desktop";
  const collapseRegistry = hasRecord && !isDesktop;
  if(shell) shell.classList.toggle("is-editor-active", collapseRegistry);
  if(registryCard) registryCard.classList.toggle("hidden", collapseRegistry);
  if(editorCard) editorCard.classList.toggle("hidden", !hasRecord);
  if(title) title.textContent = hasRecord ? (record.name || tOr("anamnesis_patient_title", "Patient anamnesis")) : tOr("anamnesis_patient_title", "Patient anamnesis");
...
function scoreMedicalRow(row, query, langField){
  ...
  const languageAlternatives = LANGUAGE_FIELD_EQUIVALENTS[langField] || [langField];
  for(const field of languageAlternatives){
...
function searchPharmacology(query){
  if(!pharmacologyState.loaded || pharmacologyRecords.length === 0) return [];
  const normalizedQuery = normalizeSearchText(query);
  ...
  for(const record of pharmacologyRecords){
    let score = 0;
    score = Math.max(score, scoreSearchValue(record.english_name, normalizedQuery, 100, 80, 60));
    score = Math.max(score, scoreSearchValue(record.slovak_name, normalizedQuery, 100, 80, 60));
    score = Math.max(score, scoreSearchValue(record.drug_id, normalizedQuery, 85, 70, 55));
    ...
    score = Math.max(score, scoreSearchValue(record.mechanism_of_action, normalizedQuery, 30, 30, 30));
    score = Math.max(score, scoreSearchValue(record.indications, normalizedQuery, 30, 30, 30));
    addHit(record, score);
  }

  return [...resultsByDrugId.values()].sort((a, b)=>{
    if(b.score !== a.score) return b.score - a.score;
    return String(a.row.english_name || a.row.drug_id).localeCompare(String(b.row.english_name || b.row.drug_id));
  });
}
...
function collectMainSearchResults(query, selectedGroup, langField, userField){
  ...
  if(selectedGroup === "all"){
    for(const trow of getLocalTerms()){
      const score = scoreUserRow(trow, query, userField);
      if(score > 0){
        results.push({ kind: "user", row: trow, score });
      }
    }
  }
  ...
  const sorted = results.sort((a, b)=>{
    const aScore = Number(a.score || 0) + (a.kind === "pharmacology" ? pharmacologyBias : 0);
    const bScore = Number(b.score || 0) + (b.kind === "pharmacology" ? pharmacologyBias : 0);
    if(bScore !== aScore) return bScore - aScore;
    if(a.kind !== b.kind){
      if(a.kind === "pharmacology") return -1;
      if(b.kind === "pharmacology") return 1;
    }
    const aLabel = a.kind === "pharmacology"
      ? String(a.row && (a.row.english_name || a.row.drug_id) || "")
      : String(a.row && (a.row[langField] || a.row[userField] || a.row.english_translation || a.row.english || "") || "");
    const bLabel = b.kind === "pharmacology"
      ? String(b.row && (b.row.english_name || b.row.drug_id) || "")
      : String(b.row && (b.row[langField] || b.row[userField] || b.row.english_translation || b.row.english || "") || "");
    return aLabel.localeCompare(bLabel);
  });

  return {
    results: sorted.slice(0, SEARCH_MAX_RESULTS),
    truncated: sorted.length > SEARCH_MAX_RESULTS
  };
}
...
async function init(){
  ...
  on('anamnesis-close-patient', 'click', ()=> closeActiveAnamnesisPatient());
  ...
  if(start === "screen-muscle-training"){
    await ensureMusclesLoaded();
    renderMuscleRegionList();
  }
...
function startQuiz({ retryItems = null, configOverrides = null } = {}){
  ...
  const pool = getQuizStudioFilteredCandidates(cfg);
  cfg.questionCount = clampQuizQuestionCount(cfg.questionCount, pool.length || 1);
