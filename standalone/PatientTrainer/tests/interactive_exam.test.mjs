...
import { PHYSICAL_EXAM_TOOLS, getPhysicalExamMap } from '../src/data/physicalExamMap.v6.js';
...
const panelCode = fs.readFileSync(path.join(root, 'src/ui/clinicalEncounterPanels.v6.js'), 'utf8');
...
// PT-v4 cache-proof, posterior-view and technique-position validation
{
  ...
  const interactive = fs.readFileSync(path.join(root, 'src/ui/interactivePhysicalExam.v6.js'), 'utf8');
  assert.ok(interactive.includes('peter_novak_back_52_v6.png'), 'Posterior photograph must be wired into the v4 examination renderer');
  assert.ok(interactive.includes("data-exam-view=\"back\""), 'Posterior toggle must remain rendered');
  assert.ok(fs.existsSync(path.join(root, 'assets/peter_novak_back_52_v6.png')), 'Posterior photograph asset must exist');
  const main = fs.readFileSync(path.join(root, 'src/main.v12.js'), 'utf8');
  assert.ok(main.includes('clinicalEncounterPanels.v6.js'), 'Main module must use the cache-busted v5 examination chain');
  console.log('✓ PT-v6 posterior and technique-coordinate validation');
}
